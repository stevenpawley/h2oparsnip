...
#' Reads an H2O model from file and inserts it back into a previously fitted
...
#'   model when using the workflows package, or a `model_spec` object from a
#'   fitted model when using the parsnip package directly.
#' @param filename A `character` specifying the file path used to read the
#'   model.
#'
#' @return The parsnip `model_spec` object or workflows `workflow` object with
